...
public class ApplicationController {
    ...
    public ResponseEntity<List<ShowApplicationDTO>> getAllApplications() {
        try {
            ...
            // Здесь можно выполнять логирование или обработку ошибки
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
        }
    }

    @GetMapping("/user")
    public ResponseEntity<List<ShowApplicationDTO>> getAllApplicationsByUser() {
        List<ShowApplicationDTO> applications = applicationService.findAllByUserId();
        return ResponseEntity.ok(applications);
    }
}
